  ...
  def _annotate_documents_single_pass(
    ...
    try:
      ...
      for batch in batch_iter:
        ...
        # storyxray 插桩：批量 infer 是 70 秒里的大头嫌疑，单独计时
        import time as _time, logging as _logging
        _t_infer = _time.monotonic()
        outputs = self._language_model.infer(batch_prompts=prompts, **kwargs)
        if not isinstance(outputs, list):
          outputs = list(outputs)
        _logging.getLogger('storyxray').info(
            '[lx-timing] batch=%d infer 完成: %d 个 prompt 耗时 %.2fs',
            _batch_idx, len(prompts), _time.monotonic() - _t_infer,
        )

        _t_post = _time.monotonic()
        _post_resolve_ms = 0.0
        _post_align_ms = 0.0
        for text_chunk, scored_outputs in zip(batch, outputs):
          ...
          _post_resolve_ms += (_time.monotonic() - _t1) * 1000.0
          ...
          _post_align_ms += (_time.monotonic() - _t2) * 1000.0
        ...
        _logging.getLogger('storyxray').info(
            '[lx-timing] batch=%d 后处理完成: resolve=%.0fms align=%.0fms 总=%.2fs',
            _batch_idx, _post_resolve_ms, _post_align_ms,
            _time.monotonic() - _t_post,
        )
